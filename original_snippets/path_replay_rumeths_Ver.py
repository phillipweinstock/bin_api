...
def turn_180(mc):
    """Rotate both motors clockwise for TURN_TIME seconds."""
    print("🔄 Turning 180° (both motors clockwise)...")
    mc.set_speed(1, 380*-1)   # both motors forward (clockwise)
    mc.set_speed(2, 380*MOTOR_DIR_R)
    time.sleep(10)
    mc.set_speed(1, 0)
    mc.set_speed(2, 0)
    mc.coast_now()
    print("✅ 180° turn complete.\n")
...
def main():
    ...
    # rows180 = load_samples(CSV_PATH180)
    rowsReverse = list(reversed(rows))
    ...
    try:
        ...
        time.sleep(0.2)
        mc.coast_now()
        print(" Path replay forward  complete.")

        servo1.angle = 0       # close the lid
        time.sleep(10) # keep lid open for 5 seconds
        servo1.angle = 180

        turn_180(mc)

        for i in range(1, len(rowsReverse)):
                t0, R0, L0 = rows[i - 1]
                t1, R1, L1 = rows[i]
                dt = max((t1 - t0) / 1000.0, MIN_DT)
        ...
        time.sleep(0.2)
        mc.coast_now()
        print(" Path replay reverse complete.")
